...
import bannerImg from '../../../public/assets/img/blog/banner.jpg';
...
const PaySidebar = ({
  ...
  const handleCreateOrder = () => {
    // Crear la orden y redirigir a la página de pago
    const orderDetails = {
      exams: selectedExams,
      totalUSD,
      totalBs,
    ...
    // Codifica los parámetros de la orden en una URL
    const queryParams = new URLSearchParams({
      exams: JSON.stringify(orderDetails.exams),
      totalUSD: totalUSD.toFixed(2),
      totalBs: totalBs.toFixed(2),
    }).toString();

    // Redirige a la página pay-order pasando los parámetros
    router.push(`/pay-order?${queryParams}`);
